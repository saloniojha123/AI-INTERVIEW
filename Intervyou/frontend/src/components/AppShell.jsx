...
export default function AppShell() {
  ...
  async function handleDeleteSession(event, sessionId, label) {
    event.preventDefault();
    event.stopPropagation();
    if (deletingSessionId) return;
    if (!window.confirm(`Delete this ${label || "interview"} report? This cannot be undone.`)) return;

    setDeletingSessionId(sessionId);
    try {
      const res = await authFetch(`/api/interview/${encodeURIComponent(sessionId)}`, { method: "DELETE" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data.success) throw new Error(data.message || "Could not delete interview");
      setHistoryItems((items) => items.filter((item) => item.sessionId !== sessionId));
      if (location.pathname.includes(sessionId)) navigate("/");
    } catch (error) {
      window.alert(error.message || "Could not delete interview");
    } finally {
    ...
    <div className="flex h-screen w-full bg-[#0f172a] text-slate-100 font-sans overflow-hidden">
      ...
      >
        ...
        <div className="flex h-16 items-center justify-between px-5 border-b border-slate-800/80">
          <Link
            to="/"
            aria-label="Go to IntervYou AI home"
            title="IntervYou AI home"
            onClick={(event) => {
              event.stopPropagation();
              navigate("/");
            }}
            className="relative z-10 flex cursor-pointer items-center gap-2.5 overflow-hidden"
          >
            ...
            {sidebarOpen && (
              <span className="text-base font-bold tracking-tight text-slate-100 truncate">
                IntervYou <span className="text-blue-400">AI</span>
              </span>
            )}
          </Link>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
          ...
          {loadingHistory ? (
            ...
            historyItems.map((item) => {
              ...
              const sessionDate = item.createdAt ? new Date(item.createdAt) : null;
              const dateLabel = sessionDate && !Number.isNaN(sessionDate.getTime())
                ? sessionDate.toLocaleDateString([], { day: "2-digit", month: "short", year: "numeric" })
                : "Recent";
              const timeLabel = sessionDate && !Number.isNaN(sessionDate.getTime())
                ? sessionDate.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })
                : "";
              const sessionTitle = `${item.role || "Mock Interview"} · ${dateLabel} ${timeLabel}`.trim();
              return (
                <div
                  key={item.sessionId || item._id}
                  role="link"
                  tabIndex={0}
                  onClick={() => navigate(`/report/${item.sessionId || item._id}`)}
                  onKeyDown={(event) => { if (event.key === "Enter") navigate(`/report/${item.sessionId || item._id}`); }}
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    {sidebarOpen && (
                      <div className="truncate">
                        ...
                        <p className="flex items-center gap-1 text-[10px] text-slate-500" title={item.resumeOriginalName || "Resume not available"}>
                          <Clock size={10} />
                          {item.resumeOriginalName || "Resume not available"}
                        </p>
                      </div>
                    )}
                  </div>
                  {sidebarOpen && (
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        disabled={deletingSessionId === (item.sessionId || item._id)}
                        onClick={(event) => handleDeleteSession(event, item.sessionId || item._id, item.role)}
                        className="rounded-md p-1 text-slate-600 transition hover:bg-red-500/10 hover:text-red-400 disabled:opacity-50"
                      >
                        <Trash2 size={13} />
                      </button>
