    ...
    <div className="print-report min-h-screen bg-[#0f172a] p-6 font-sans text-slate-100 lg:p-10">
      <div className="mx-auto flex max-w-5xl flex-col gap-8">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {scorecards.map((card) => (
            <div key={card.label} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 transition hover:border-slate-700 hover:bg-slate-900">
              <span className="block min-h-8 text-[10px] font-semibold uppercase leading-4 tracking-wider text-slate-400">{card.label}</span>
              <div className="my-2 flex items-baseline gap-1.5">
                <span className={`text-2xl font-extrabold ${card.score === null ? "text-slate-500" : card.color}`}>{card.score === null ? "N/A" : card.score}</span>
